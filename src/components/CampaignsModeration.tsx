...
import CampaignPitchesModal from './CampaignPitchesModal';
...
const CampaignsModeration: React.FC = () => {
    ...
    // Applicants drill-down modal — { id, title } or null when closed. Opens
    // when the pitches_count chip is clicked. Added 2026-08-25.
    const [pitchesModal, setPitchesModal] = useState<{ id: string; title: string } | null>(null);
    ...
    return (
        <div className="text-gray-100">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex gap-4 flex-wrap text-sm text-gray-400 mb-3">
                                        ...
                                        {typeof c.pitches_count === 'number' && (
                                            c.pitches_count > 0 ? (
                                                <button
                                                    onClick={() => setPitchesModal({ id: c._id, title: c.title })}
                                                    className="text-primary hover:underline"
                                                >
                                                    {c.pitches_count} applicant{c.pitches_count === 1 ? '' : 's'} →
                                                </button>
                                            ) : (
            ...
            {/* Applicants drill-down modal — shows which creators applied to
                the campaign. Read-only view; hire/reject stays on the brand side. */}
            {pitchesModal && (
                <CampaignPitchesModal
                    campaignId={pitchesModal.id}
                    campaignTitle={pitchesModal.title}
                    onClose={() => setPitchesModal(null)}
                />
            )}
